...
<script>
    ...
    .then(data => {
        ...
        if (data.success) {
            console.log("kkmknjn");
            alertBox.innerHTML = `<div class="bg-green-200 text-green-700 p-3 rounded">${data.message}</div>`;
            document.getElementById("registerForm").reset(); // Clear form fields
        } else {
            console.log("jkkm");
            alertBox.innerHTML = `<div class="bg-red-200 text-red-700 p-3 rounded">${data.message}</div>`;
        }
    })
